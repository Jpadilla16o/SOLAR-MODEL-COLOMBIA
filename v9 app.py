...
# --- LÓGICA DE CÁLCULO ACTUALIZADA (CAPACIDAD REAL) ---
if st.session_state.registro_exitoso:
    eficiencia = 0.80
    kwp_teorico = (consumo_mes / 30) / (hsp * eficiencia)
    
    # 1. Determinamos número de paneles primero
    potencia_panel = 550
    num_paneles = math.ceil((kwp_teorico * 1000) / potencia_panel)
    
    # 2. CAPACIDAD REAL INSTALADA (Basada en paneles físicos)
    kwp_instalado = (num_paneles * potencia_panel) / 1000
    
    # 3. Costo basado en la capacidad real que se va a montar
    if kwp_instalado < 3: costo_kwp = 6500000
    elif 3 <= kwp_instalado < 15: costo_kwp = 5000000
    elif 15 <= kwp_instalado < 100: costo_kwp = 4000000
    else: costo_kwp = 3800000

    # Ingeniería Detallada (Basada en kwp_instalado)
    ...
    # Finanzas (Basadas en kwp_instalado)
    inversion_total = kwp_instalado * costo_kwp
    ...
    with tab2:
        ...
        t1.metric("Capacidad Instalada", f"{kwp_instalado:.2f} kWp", help="Capacidad real basada en el número físico de paneles.")
        ...
        with col_ta:
            ...
            st.markdown(f"- **Inversor:** {'Monofásico' if kwp_instalado < 8 else 'Trifásico'} de {kwp_instalado:.1f} kW.\n- **Tecnología:** Módulos Tier 1 N-Type.\n- **Estructura:** Aluminio anodizado.")
        with col_tb:
    ...
    with tab3:
        ...
        pdf_bytes = generar_pdf({
            ...
            "Ciudad": ciudad, 
            "Capacidad Instalada": f"{kwp_instalado:.2f} kWp",
